...
"/usr/local/lib";
#else
"C:\\Program Files\\Saxonica\\SaxonHEC 12.3";
#endif*/

char *getResourceDirectory() { return resources_dir; }
...
int detach_graalvm_thread(sxnc_environment *env) {
  /*graal_isolatethread_t *curthread =
  graal_get_current_thread(((env)->isolate)); if(!curthread) { fprintf(stdout,
  "Cur detach thread pointer not in isolate \n"); fflush(stdout); return 1;
  }
...
void setParameter(sxnc_parameter **parameters, int *parLen, int *parCap,
  ...
  if ((*parLen) >= (*parCap)) {
    (*parCap) *= 2;
    sxnc_parameter *temp =
        (sxnc_parameter *)malloc(sizeof(sxnc_parameter) * (*parCap));
    int i = 0;
...
void setProperty(sxnc_property **properties, int *propLen, int *propCap,
  ...
  if (((*propLen) + 1) >= (*propCap)) {
    (*propCap) *= 2;
    sxnc_property *temp =
        (sxnc_property *)malloc(sizeof(sxnc_property) * (*propCap));
    int i = 0;
